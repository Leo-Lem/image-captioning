...
def predict(model: Decoder, image_name: str) -> str:
    """ Predicts a caption for an image. """
    features = ImagePreprocessor()(image_name)
    prediction = model.predict(features.unsqueeze(0))
...
def display(image_name: str, caption: str, true_caption: str = None, ax: pyplot.Axes = None):
    ...
    if ax:
        ...
        ax.set_title(caption
                     if true_caption is None else f"Predicted: {caption}\nTrue: {true_caption}", fontsize=8, pad=10)
    else:
        pyplot.imshow(image)
...
def predictions(model: Decoder, data: CaptionedImageDataset, n: int):
